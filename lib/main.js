...
export default class Depsify extends Transform {
  constructor(opts = {}) {
    ...
    this.processor = []
    ;[].concat(opts.processor).filter(Boolean).forEach(this.addProcessor, this)
  ...
  _transform(row, _, next) {
    if (row.processor) {
      this.addProcessor(row.processor)
      return next()
    }
  ...
  addProcessor(processor) {
    ...
    if (Array.isArray(processor) && typeof processor[0] === 'function') {
      this.processor.push(function (result) {
        let args = processor.slice(1)
        args.unshift(result)
        return processor[0].apply(null, args)
      })
    }
  }
  readFile(file) {
