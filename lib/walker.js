...
function Walker(opts) {
  opts = this.opts = createOptions(opts)
  if (opts.processor.length) {
    var processor = postcss(opts.processor)
    opts.transform.push(function (result) {
      return processor.process(result.root, { from: result.from })
        .then(function (style) {
          result.root = style.root
        })
    })
  }
}
